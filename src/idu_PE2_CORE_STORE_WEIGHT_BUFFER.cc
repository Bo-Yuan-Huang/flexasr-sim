...
auto& univ_var_906 = local_var_16;
return univ_var_906;
}
void flex::update_flex_PE2_CORE_STORE_WEIGHT_BUFFER() {
std::map<sc_biguint<32>, sc_biguint<8>> local_var_0;
store_907(local_var_0);
for (auto& it : local_var_0) {
  flex_pe2_core_weight_buffer[it.first] = it.second;
}
#ifdef ILATOR_VERBOSE
instr_update_log << "No." << std::dec << GetInstrCntr() << '\t' << "PE2_CORE_STORE_WEIGHT_BUFFER state updates:" << std::endl;
instr_update_log << "    flex_pe2_core_weight_buffer get updated" << std::endl;
